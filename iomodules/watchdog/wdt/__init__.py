...
def wdt_setRepowerOnBattery(val):
    ret = -1
    bus = smbus.SMBus(1)
    if val != 0:
        val = 0
...
def wdt_setPowerButtonEnable(val):
    ret = -1
    bus = smbus.SMBus(1)
    if val != 0:
        val = 1
